...
Certificates are checked on every run. To force renewal, use tags:
...
- Review the [Architecture](Architecture) page for how roles interact
- Browse the role reference pages for all available variables: [elasticsearch](Roles-elasticsearch), [kibana](Roles-kibana), [logstash](Roles-logstash), [beats](Roles-beats), [elasticstack](Roles-elasticstack), [repos](Roles-repos)
