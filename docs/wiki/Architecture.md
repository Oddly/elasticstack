...
Templates switch based on `elasticstack_release | int >= 9`. No user action needed — set `elasticstack_release` and the correct config is generated.

## Password and secret defaults

All roles ship with placeholder passwords that must be changed in production:
