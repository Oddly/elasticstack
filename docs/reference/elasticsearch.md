...
The certificate file inputs below are used when `elasticsearch_cert_source: external`. `elasticsearch_certs_dir` also controls the destination and rendered paths for certificates generated by the role.
...
`elasticsearch_certs_dir` is the directory on the managed node where the role stores generated or external TLS certificates. It defaults to `/etc/elasticsearch/certs`; set it when the package configuration or filesystem layout uses another location. The generated `elasticsearch.yml` uses the configured directory for every transport and HTTP certificate path, regardless of whether the certificates are generated or externally supplied.

`elasticsearch_http_ssl_keystore_path` overrides the HTTP SSL keystore path written to `elasticsearch.yml`. When empty, the role uses the generated node keystore under `elasticsearch_certs_dir`. Use this when you need to point to a different keystore location.
