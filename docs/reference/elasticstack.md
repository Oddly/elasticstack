...
The shared `fetch_password.yml` task extracts a specific user's password with `awk` and registers it as an Ansible fact. It fails when the requested user is absent or has no password value.
