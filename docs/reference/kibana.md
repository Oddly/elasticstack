...
The certificate file inputs below are used when `kibana_cert_source: external`. `kibana_certs_dir` also controls the destination and rendered paths for certificates generated by the role.
...
`kibana_certs_dir` is the directory on the managed node where the role stores generated or external TLS certificates. It defaults to `/etc/kibana/certs`; set it when your Kibana package or filesystem layout uses another location. The generated `kibana.yml` uses this directory for its CA and web TLS paths in both certificate modes.
...
1. On the Kibana node itself (`{{ kibana_certs_dir }}/`)
