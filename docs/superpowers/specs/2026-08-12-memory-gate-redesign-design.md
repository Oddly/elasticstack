...
- Synthetic tests for the gate script before any real-tree run (shell
  harness faking `incus list` output and `/proc/meminfo`; assert
  admission math, FIFO order, no-harm bypass, ticket GC, conversion,
  fail-fast verdict). This repo's practice: static/coordination logic
  gets adversarial synthetic cases first.
- One labeled `ci:run` PR exercising the standard matrix plus
  full-stack, watching `/tmp/molecule-gate/` and gate log lines.
- Observe the next Tue/Thu/Sat nightly storm for: zero cgroup OOMs in
  the kernel journal, no gate-starved failures, full-stack wall clock
  vs the max-parallel:3 baseline.

## Rollout

One PR, two commits: (1) gate rewrite + create.yml surgery + container
limit bumps (atomic — nothing external needs to stay in sync);
(2) `max-parallel` 3 → 6, independently revertable.
